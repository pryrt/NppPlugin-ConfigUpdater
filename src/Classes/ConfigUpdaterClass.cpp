...
void ConfigUpdater::_initThemeValidatorXSD(void)
{
	...
	if (!PathFileExists(_wsThemeValidatorXsdFileName.c_str())) {
		...
		std::wstring wsContents = LR"myMultiLineXsd(<?xml version="1.0" encoding="UTF-8" ?>
  ...
  <xs:element name="NotepadPlus">
    <xs:complexType>
      <xs:sequence>
		...
		DWORD bytesToWrite = static_cast<DWORD>(wsContents.size() * sizeof(wchar_t));
		BOOL success = WriteFile(hFile, wsContents.c_str(), bytesToWrite, &bytesWritten, NULL);
		if (!success) {
			DWORD errNum = GetLastError();
			LPWSTR messageBuffer = nullptr;
			FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
...
bool ConfigUpdater::_updateOneTheme(tinyxml2::XMLDocument* pModelStylerDoc, std::wstring themeDir, std::wstring themeName, bool isIntermediateSorted)
{
	...
	//bool isValid =
	ValidateXML::validate_xml(themePath.c_str(), _wsThemeValidatorXsdFileName);
	//if (false && !isValid) {
	//	std::wstring msg = std::wstring(L"Validation of ") + themePath + std::wstring(L" failed:\n\n") + ValidateXML::wsGetValidationMessage();
	//	::MessageBox(nullptr, msg.c_str(), L"Theme Validation Failed", MB_ICONWARNING);
	//}
